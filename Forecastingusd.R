#------------------------------------------------------------------------
#Forecasting the RMB against USD
#------------------------------------------------------------------------
rm(list = ls(all = TRUE))
graphics.off()
setwd("")
...
source("UYCTR.r")
e     = read_excel("FPMMUSChinaFree.xlsx")
...
pred.3    = rep(0, b1)
sl        = rep(0, 300)
beta.fi   = matrix(0, nrow=b1, nco=8)
i.0       = rep(0, length(L))
...
for(t in b0:b1){
  ...
  c.1  = matrix(0, nrow =length(L), ncol = 8)
  beta = matrix(0, nrow =length(L), ncol = 8)
...
plot(time[c((b0+h+1):(b1+h+1))] ,   pred[(b0+h):(b1+h)],  lwd = 10, ylim=c(1.80, 2.0), type = "n", xaxt='n', xlab='', ylab='log')
...
legend(time[c(125)],2.0, c("Best competitor","True value", "UYCTR"),lty=c(1,1,1), lwd =c(3,3,3), pch=c('','',''), col=c(2,1,4), cex=1.1, bty='n')
